...
import country_codes from 'iso-3166-1';
/* import type {
  CurrencyCodes,
  UppercaseTwoLetterCountryCodes,
...
export default function (input: string, field: string) {
  ...
  if (
    normalized_input.match(SHA256_REGEX) ||
    normalized_input.match(MD5_REGEX)
  ) {
  ...
  switch (field) {
    case 'country':
      normalized_input = normalizeCountry(normalized_input);
      break;
    case 'ct':
      normalized_input = normalizeCity(normalized_input);
      break;
    case 'em':
      normalized_input = normalizeEmail(normalized_input);
      break;
    case 'ge':
      normalized_input = normalizeGender(normalized_input);
      break;
    case 'ph':
      normalized_input = normalizePhone(normalized_input);
      break;
    case 'st':
      normalized_input = normalizeState(normalized_input);
      break;
    case 'zp':
      normalized_input = normalizeZip(normalized_input);
      break;
    case 'f5first':
    case 'f5last':
      normalized_input = normalizeF5NameField(normalized_input);
      break;
    case 'fi':
      normalized_input = normalized_input.charAt(0);
      break;
    case 'dobd':
      normalized_input = normalizeDobd(normalized_input);
      break;
    case 'dobm':
      normalized_input = normalizeDobm(normalized_input);
      break;
    case 'doby':
      normalized_input = normalizeDoby(normalized_input);
      break;
    case 'currency':
      normalized_input = normalizeCurrency(normalized_input);
      break;
  }
...
const normalizeCountry = (country: string): string => {
  if (!country_codes.whereAlpha2(country)) {
    throw new Error(
      `Invalid country code: '${country}'. Please follow ISO 3166-1 2-letter standard for representing country. eg: US`,
    );
  }
...
const normalizeCurrency = (currency: string): string => {
  ...
  if (!currency_codes.codes().includes(currency.toUpperCase())) {
    throw new Error(
      `Invalid format for currency:'${currency}'.Please follow ISO 4217 3-letter standard for representing currency. Eg: usd`,
    );
  }
...
// https://github.com/facebook/facebook-nodejs-business-sdk/blob/3edb78dc855ca348fd0291af930c035c03347049/src/objects/serverside/utils.js#L34
...
const normalizeEmail = (email: string): string => {
  ...
  if (!EMAIL_RE.test(email)) {
    throw new Error(
      `Invalid email format for the passed email:'${email}'.Please check the passed email format.`,
    );
  }
 ...
 */
const normalizeF5NameField = (name: string): string => {
 ...
 */
const normalizePhone = (phone_number: string): string => {
  ...
  if (isInternationalPhoneNumber(phone_number)) {
    phone_number = phone_number.replace(PHONE_NUMBER_DROP_PREFIX_ZEROS, '');
  }

  if (phone_number.length < 7 || phone_number.length > 15) {
    throw new Error(
      "Invalid phone number format for the passed phone number:'" +
        phone_number +
        "'.Please check the passed phone number format.",
    );
  }
 ...
 */
const normalizeDobd = (dobd: string): string => {
  if (dobd.length === 1) {
    dobd = '0' + dobd;
  }
  ...
  if (dobd_int < 1 || dobd_int > 31) {
    throw new Error(
      "Invalid format for dobd:'" + dobd + "'.Please use 'DD' format for dobd.",
    );
  }
 ...
 * @return {String} Normalized value.
 */
const normalizeDobm = (dobm: string): string => {
  if (dobm.length === 1) {
    dobm = '0' + dobm;
  }
  ...
  if (dobm_int < 1 || dobm_int > 12) {
    throw new Error(
      "Invalid format for dobm:'" + dobm + "'.Please use 'MM' format for dobm.",
    );
  }
 ...
 * @return {String} Normalized value.
 */
const normalizeDoby = (doby: string): string => {
  if (!doby.match(/^[0-9]{4}$/)) {
    throw new Error(
      "Invalid format for doby:'" +
        doby +
        "'.Please use 'YYYY' format for doby.",
    );
  }
 ...
 */
const isInternationalPhoneNumber = (phone_number: string): boolean => {
